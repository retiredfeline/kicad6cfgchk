...
def platform_defaults():
    ...
    if PLATFORM == 'linux':
        ...
        cfgdir = home / 'kicad' / 'lib' / '7.0'
        SYM_LIB_TABLE['path'] = '\\Program Files\\KiCad\\7.0\\share\\kicad\\symbols'
        FP_LIB_TABLE['path'] = '\\Program Files\\KiCad\\7.0\\share\\kicad\\footprints'
    else:
...
def chk_lib_table(cfgdir, table_info, envvars):
    ...
    version = sexpdata.car(tail)
    if version[0].value() != 'version' or version[1] != 7:
        print(f'No version 7 marker')
        return {}
    tail = sexpdata.cdr(tail)
    while tail:
